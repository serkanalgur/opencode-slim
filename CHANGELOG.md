...
### Added
- **v1/v2 API Compatibility** - TUI plugin now works with both legacy and modern OpenCode APIs
  - Fallback to `api.command.register()` for OpenCode < 1.18.29
  - Use `api.keymap.registerLayer()` for OpenCode >= 1.18.29
- **Manual Installation** - Added fallback installation instructions for config file

### Changed
- Removed unused `lib/tui/commands.ts` (consolidated into tui.tsx)
- Updated `/compress` slash command to show usage instructions
- Updated README with clearer installation and usage docs

### Fixed
- Fixed `api.ui.toast.show()` → `api.ui.toast()` API call
